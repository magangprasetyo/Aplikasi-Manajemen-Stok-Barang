    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
        <div class="items-center justify-between lg:flex">
            <div class="mb-4 lg:mb-0">
                <h3 class="mb-2 text-xl font-bold text-gray-900 dark:text-white">Data Categori</h3>
                <span class="text-base font-normal text-gray-500 dark:text-gray-400">Ini adalah daftar terbaru
                    Data Produk</span>
            </div>
        </div>
        <a href="{{ route('tambah-categori') }}" class="inline-block px-4 py-2 text-white bg-green-600 rounded hover:bg-green-700">
            Tambah
        </a>
                                ...
                                @foreach ($categori as $item)
                                    <tr class="@if ($loop->even) bg-gray-50 dark:bg-gray-700 @endif">
                                        ...
                                        <td class="p-4">
                                            ...
                                            {{-- Tombol Lihat --}}
                                            <a href="{{ route('findCategorieById', ['id' => $item->id]) }}"
                                                class="inline-block px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700">
                                                Lihat
                                            </a>
                                            <!-- Tombol Edit -->
                                            <a href="{{ route('findUpdateCategori', ['id' => $item->id]) }}"
                                                class="inline-block px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700">
                                                Edit
                                            </a>

                                            <!-- Formulir untuk Hapus -->
                                            <form action="{{ route('deleteCategorieById', $item->id) }}" method="POST"
